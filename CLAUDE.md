...
**Readers** (`Readers/`) exist because SWSGI delivers request bodies as a chunk stream terminated by an empty `Data`. `DataReader.read` buffers to EOF; `JSONReader` and `URLParametersReader` both build on `DataReader` and add parsing plus an optional `errorHandler`. `URLParametersReader.readParameters` wraps the pairs in `FormParameters` for lookup by key; it has its own name because a `read` overload differing only in the handler's parameter type would make `{ params in ... }` calls ambiguous. Read the body via `environ.swsgi.input`, or pass `environ` straight to a reader.
