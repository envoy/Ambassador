...
`@Sendable` typealiases, so anything a `WebApp` captures in them must be `Sendable`
...
Routes are keyed by string; each key is compiled once, at registration, into an `NSRegularExpression` (stored in a private `Route` alongside its `WebApp`) and matched with `firstMatch(in:)` — so matching is **substring, not anchored**, and iteration order over the `routes` dictionary is undefined. Overlapping patterns therefore resolve non-deterministically; that is existing behavior tests depend on, so be careful "fixing" it. An invalid pattern traps in the subscript setter, at the registration site. Capture groups are injected into the environ under `SWSGIEnvironment.routerCapturesKey` (`"ambassador.router_captures"`) as `[String]`, readable via `environ.swsgi.routerCaptures`; an optional group that didn't participate captures `""`. The subscript and the route snapshot taken during matching are both guarded by an `NSLock`, because routes are typically registered from a test thread while the event loop reads them. Assigning `nil` to the subscript removes the route. The path is read as `environ.swsgi.pathInfo!`: a missing `PATH_INFO` is a broken server, not a 404.

## Conventions

- Response types are `struct`s conforming to `WebApp`; readers are caseless `enum`s (static-only namespaces); `Router` is the lone `open class`.
- `JSONResponse(json:)` and `JSONResponse(encoding:)` take `@autoclosure`s so the payload is built per request, like a handler's body. Keep that: tests set state after registering routes.
